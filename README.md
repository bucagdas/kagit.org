...
- **Çok dilli arayüz**: Türkçe, Almanca, Azerbaycanca, İngilizce. Dil, tarayıcı diline göre otomatik seçiliyor
  ...
  değiştirilebiliyor.
- **Okunduktan sonra sil (burn after read)**: bir kağıt, ilk gerçek okumadan hemen sonra kalıcı olarak
...
1. Kağıdınızı 4 karakterlik kısa bir URL ile, ya da kendi seçtiğiniz bir isimle paylaşın.
...
1. Okunduktan sonra otomatik silinen (burn after read) kağıtlar.

## Kullanım

1. Kağıdınızı doğrudan site üzerinden ([kagit.org](https://kagit.org)) oluşturabilir, güncelleyebilir,
   silebilirsiniz.
...
4. [doc/skill.md](doc/skill.md) - API'nin AI ajanlarına yönelik, özet bir paketlemesi. Kodlama ajanınıza verin,
   kağıt yükleyip indirebilsin ve yönetebilsin.
...
- **[Workers KV](https://developers.cloudflare.com/kv/platform/pricing/)** - küçük kağıtlar ve
  kağıt-başı metadata.
...
- **[R2](https://developers.cloudflare.com/r2/pricing/)** - `R2_THRESHOLD` üzerindeki kağıdın içeriği. Egress
...
- **Ücretsiz katman - kişisel bir pastebin.** Sınırlayıcı olan KV yazmaları (**günde 1 bin yükleme**) ve
  KV/Workers okumaları (**günde ~100 bin erişim**); R2'de küçük kağıtlar için **1 GB**, büyük dosyalar için
...
> [!NOTE]
> Küçük kağıtlar (R2 değil) KV'ye gider, böylece çöp toplama ucuz kalır. KV anahtar bazlı son kullanma
> tarihini destekliyor, süresi dolan kağıtlar kendiliğinden kayboluyor. R2'nin yerleşik bir son kullanma
